...
from context_engineering import (
    ContextItem, Budget, pack, ScoringWeights, 
    StructuralSegmenter, SemanticSegmenter, BoundaryProtector
)
...
async def main():
    ...
    tester.add_case(EvalCase(
        ...
        items=[
            ContextItem(id="on", content="Reactor is ENABLED.", priority=9.0, embedding=[1.0, 0.0]),
            ContextItem(id="off", content="Reactor is DISABLED.", priority=9.0, embedding=[1.0, 0.01]),
        ],
        budget=1000,
        required_ids={"on", "off"}
    ))
    ...
    tester.add_case(EvalCase(
        ...
        items=[
            ...
            ContextItem(id="key", content="SECRET KEY 12345 " * 20, priority=10.0, parent_id="root"), # ~60 tokens
        ],
        budget=70, 
        required_ids={"key"},
        disallowed_ids={"root"}
    ...
    tester.add_case(EvalCase(
        ...
        items=[
            ...
            ContextItem(id="B", content="Linked B " * 20, priority=1.0, links=["A"]), # ~40 tokens
            ContextItem(id="noise", content="High Priority Noise " * 20, priority=8.0), # ~40 tokens
        ],
    ...
    for s in segments:
        if "v4.12.0" in s.content and "rc1" not in s.content: split_detected = True
        if "550e8400" in s.content and "446655440000" not in s.content: split_detected = True
    
    if split_detected: print("FAILED: Protected entity was split.")
    else: print("PASSED: Protected entities remained atomic.")
