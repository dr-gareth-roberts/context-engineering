...
class TestPipeline:
    ...
    def test_budget_object(self):
        result = create_pipeline(Budget(max_tokens=500)).add(make_item("a", "system", 10, 50)).build()
        assert len(result.selected) == 1

    def test_multiple_items(self):
        result = (create_pipeline(500)
