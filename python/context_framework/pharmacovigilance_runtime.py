...
@dataclass(slots=True)
class HTTPFAERSAdapter(_HTTPJSONAdapterBase):
...
@dataclass(slots=True)
class HTTPLotTraceabilityAdapter(_HTTPJSONAdapterBase):
...
@dataclass(slots=True)
class HTTPSafetyActionAdapter(_HTTPJSONAdapterBase):
...
@dataclass(slots=True)
class PharmacovigilanceCommander:
    ...
    def _execute_tasks(self, tasks: list[_ExecutionTask]) -> list[PVActionResult]:
        if not tasks:
            return []
    ...
    def _execute_task(self, task: _ExecutionTask) -> PVActionResult:
        if task.idempotency_key and self.idempotency_store.seen(task.idempotency_key):
            return PVActionResult(
