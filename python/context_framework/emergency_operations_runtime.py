...
@dataclass(slots=True)
class HTTPHazardIntelAdapter(_HTTPJSONAdapterBase):
...
@dataclass(slots=True)
class HTTPLogisticsCapacityAdapter(_HTTPJSONAdapterBase):
...
@dataclass(slots=True)
class HTTPEOCActionAdapter(_HTTPJSONAdapterBase):
...
@dataclass(slots=True)
class EmergencyOperationsCommander:
    ...
    def _execute_tasks(self, tasks: list[_ExecutionTask]) -> list[EOCActionResult]:
        if not tasks:
            return []
    ...
    def _execute_task(self, task: _ExecutionTask) -> EOCActionResult:
        if task.idempotency_key and self.idempotency_store.seen(task.idempotency_key):
            return EOCActionResult(
