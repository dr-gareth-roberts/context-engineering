...
@dataclass(slots=True)
class HTTPLineTelemetryAdapter(_HTTPJSONAdapterBase):
...
@dataclass(slots=True)
class HTTPMaintenanceHistoryAdapter(_HTTPJSONAdapterBase):
...
@dataclass(slots=True)
class HTTPManufacturingActionAdapter(_HTTPJSONAdapterBase):
...
@dataclass(slots=True, frozen=True)
class ManufacturingActionResult:
    integration: str
    operation: str
    target: str
    success: bool
...
@dataclass(slots=True)
class ManufacturingRootCauseCommander:
    ...
    def _execute_tasks(self, tasks: list[_ExecutionTask]) -> list[ManufacturingActionResult]:
        if not tasks:
            return []
    ...
    def _execute_task(self, task: _ExecutionTask) -> ManufacturingActionResult:
        if task.idempotency_key and self.idempotency_store.seen(task.idempotency_key):
            return ManufacturingActionResult(
