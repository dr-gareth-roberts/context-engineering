...
@dataclass(slots=True)
class HTTPSystemInventoryAdapter(_HTTPJSONAdapterBase):
...
@dataclass(slots=True)
class HTTPDependencyGraphAdapter(_HTTPJSONAdapterBase):
...
@dataclass(slots=True)
class HTTPMigrationActionAdapter(_HTTPJSONAdapterBase):
...
@dataclass(slots=True, frozen=True)
class LegacyMigrationActionResult:
    integration: str
    operation: str
    target: str
    success: bool
...
@dataclass(slots=True)
class LegacyModernMigrationCommander:
    ...
    def _execute_tasks(self, tasks: list[_ExecutionTask]) -> list[LegacyMigrationActionResult]:
        if not tasks:
            return []
    ...
    def _execute_task(self, task: _ExecutionTask) -> LegacyMigrationActionResult:
        if task.idempotency_key and self.idempotency_store.seen(task.idempotency_key):
            return LegacyMigrationActionResult(
    ...
    def _log_audit_event(
        ...
        *,
        batch_id: str,
        mode: str,
        row: LegacyMigrationActionResult,
    ) -> None:
        event = {
