...
@dataclass(slots=True)
class HTTPBedCapacityAdapter(_HTTPJSONAdapterBase):
...
@dataclass(slots=True)
class HTTPAcuityIntelAdapter(_HTTPJSONAdapterBase):
...
@dataclass(slots=True)
class HTTPClinicalActionAdapter(_HTTPJSONAdapterBase):
...
@dataclass(slots=True)
class ClinicalOperationsCommander:
    ...
    def _execute_tasks(self, tasks: list[_ExecutionTask]) -> list[ClinicalActionResult]:
        if not tasks:
            return []
    ...
    def _execute_task(self, task: _ExecutionTask) -> ClinicalActionResult:
        if task.idempotency_key and self.idempotency_store.seen(task.idempotency_key):
            return ClinicalActionResult(
