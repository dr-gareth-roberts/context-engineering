        ...
        if not query.include_expired:
            if item.ttl_seconds:
                try:
                    ...
                    if c_ms + item.ttl_seconds * 1000 <= now_ms: continue
...
class InMemoryStore(MemoryStore):
    ...
    async def consolidate(self, summarizer: Callable[[str], Awaitable[str]], salience_threshold: float = 0.3) -> int:
        cold_ids = [id for id, i in self._items.items() if (i.salience or 1.0) < salience_threshold and not i.is_summary]
        for item_id in cold_ids:
            item = self._items[item_id]
            summary = await summarizer(item.content)
            item.content = f"[Flashcard] {summary}"; item.is_summary = True; item.salience = salience_threshold + 0.1; item.updated_at = _now_iso()
        return len(cold_ids)
...
class SqliteStore(MemoryStore):
    def __init__(self, database_path: str) -> None:
        self.conn = sqlite3.connect(database_path); self._init()
    ...
    def put(self, item: MemoryItem | List[MemoryItem]):
        items = item if isinstance(item, list) else [item]; normalized = [_normalize(e) for e in items]
        with self.conn:
            for e in normalized:
                self.conn.execute("""
                ...
                """, (e.id, e.content, e.created_at, e.updated_at or e.created_at, e.last_accessed_at or e.created_at, e.salience, e.ttl_seconds, 1 if e.is_summary else 0, json.dumps(e.embedding) if e.embedding else None, json.dumps(e.metadata or {})))
        return normalized

    def get(self, item_id: str):
        self.conn.execute("UPDATE memory_items SET last_accessed_at = ? WHERE id = ?", (_now_iso(), item_id)); self.conn.commit()
        r = self.conn.execute("SELECT id, content, created_at, updated_at, last_accessed_at, salience, ttl_seconds, is_summary, embedding_json, metadata_json FROM memory_items WHERE id = ?", (item_id,)).fetchone()
        if not r: return None
        return MemoryItem(id=r[0], content=r[1], createdAt=r[2], updatedAt=r[3], lastAccessedAt=r[4], salience=r[5], ttlSeconds=r[6], isSummary=bool(r[7]), embedding=json.loads(r[8]) if r[8] else None, metadata=json.loads(r[9]) if r[9] else {})

    def query(self, query: Optional[MemoryQuery] = None):
        cursor = self.conn.execute("SELECT id, content, created_at, updated_at, last_accessed_at, salience, ttl_seconds, is_summary, embedding_json, metadata_json FROM memory_items")
        items = [MemoryItem(id=r[0], content=r[1], createdAt=r[2], updatedAt=r[3], lastAccessedAt=r[4], salience=r[5], ttlSeconds=r[6], isSummary=bool(r[7]), embedding=json.loads(r[8]) if r[8] else None, metadata=json.loads(r[9]) if r[9] else {}) for r in cursor.fetchall()]
        return _apply_query(items, query or MemoryQuery())

    def forget(self, item_id: str):
        c = self.conn.execute("DELETE FROM memory_items WHERE id = ?", (item_id,)); self.conn.commit(); return c.rowcount > 0

    async def consolidate(self, summarizer, threshold=0.3):
        cold = self.conn.execute("SELECT id, content FROM memory_items WHERE salience < ? AND is_summary = 0", (threshold,)).fetchall()
        for i_id, content in cold:
            s = await summarizer(content)
            with self.conn: self.conn.execute("UPDATE memory_items SET content = ?, is_summary = 1, salience = ?, updated_at = ? WHERE id = ?", (f"[Flashcard] {s}", threshold + 0.1, _now_iso(), i_id))
        return len(cold)
