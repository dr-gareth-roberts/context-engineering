...
function makeItems(): ContextItem[] {
  return [
    { id: "system", content: "System prompt", priority: 10, recency: 5, tokens: 10 },
    { id: "code", content: "function foo() { return 42; }", priority: 8, recency: 8, tokens: 20 },
    { id: "docs", content: "API documentation for the foo module", priority: 4, recency: 2, tokens: 25 },
    { id: "chat", content: "User asked about testing", priority: 6, recency: 9, tokens: 15 },
  ];
}
...
describe("replay", () => {
  ...
  it("replays with different scoring weights", () => {
    ...
    const priorityHeavy = report.variants.find((v) => v.name === "priority-heavy")!;
  ...
  it("calculates average utilization across recordings", () => {
    ...
    recorder.record({ model: "gpt-4o", items, budget: budget1, result: pack(items, budget1) });
    recorder.record({ model: "gpt-4o", items, budget: budget2, result: pack(items, budget2) });
