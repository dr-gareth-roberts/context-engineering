...
describe("Causal Graph-Aware Compaction", () => {
  it("should prioritize root goal and active task over closed task noise", () => {
    ...
    const graph: BeadsIssue[] = [
      { id: "root", title: "Mission", status: "open", priority: 1, issue_type: "task", created_at: "", updated_at: "" },
      { id: "task-a", title: "Old Task", status: "closed", priority: 3, issue_type: "task", created_at: "", updated_at: "" },
      { id: "task-b", title: "New Task", status: "open", priority: 2, issue_type: "task", created_at: "", updated_at: "" },
    ];
    ...
    ctx.addTurn({ role: "user", content: "GOAL: Build a house.", taskId: "root" } as any);
    
    // Turns 2-6: Noise from a closed task (taskId: task-a) - 500 tokens each
    for (let i = 0; i < 5; i++) {
      ctx.addTurn({ role: "assistant", content: `Debugging plumbing ${i}...`, taskId: "task-a" } as any);
    }

    // Turn 7: Active work (taskId: task-b) - 500 tokens
    ctx.addTurn({ role: "user", content: "Now starting on the roof.", taskId: "task-b" } as any);
  ...
  it("should protect 'Outcome' turns even if the task is closed", () => {
    ...
    const customEstimator = (text: string) => text.includes("Outcome") || text.includes("ARCHITECTURE") ? 800 : 500;
    ...
    ctx.addTurn({ role: "assistant", content: "Noise 1", taskId: "task-a" } as any);
    ctx.addTurn({ role: "assistant", content: "Noise 2", taskId: "task-a" } as any);
