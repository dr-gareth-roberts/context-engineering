...
def calculate_fcv(items: List[Bead], active_task_id: str, dependencies: List[str]) -> Dict[str, float]:
    """
...
def simulate():
    ...
    task_a_noise = [Bead(i, f"Postgres Debugging {i}", 500, "task_a", "closed") for i in range(1, 9)]
    task_a_outcome = Bead(9, "FINAL POSTGRES SCHEMA: users(id, email...)", 800, "task_a", "closed", is_outcome=True)
    
    # 3. Task B: Auth Implementation (Active)
    # 5 turns of current work
    task_b_active = [Bead(i, f"Auth Implementation {i-10}", 600, "task_b", "open", depends_on=["task_a"]) for i in range(10, 15)]
    ...
    print("ANALYSIS: Agent kept recent debugging noise but dropped the Root Goal and the DB Schema!")
